...
# Note: For production use, convert Alpine to a traditional installation using:
#   setup-disk -m sys /dev/mmcblk0
# This will install Alpine to persistent storage and eliminate diskless limitations.
if [ "$DISKLESS_MODE" = true ] && [ "$TC_OS" = "alpine" ]; then
    echo "Persisting configuration for diskless mode using lbu..."
    echo "WARNING: Diskless mode is not recommended for production."
    echo "Convert to traditional installation with: setup-disk -m sys /dev/mmcblk0"
    echo ""
...
EOF
    ...
    $SUDO lbu commit
    echo "Configuration persisted. Note: Packages may need to be reinstalled on boot."
    echo "For production, convert to traditional installation with: setup-disk -m sys /dev/mmcblk0"
fi
